...
| AGENT-RUNTIME-001..004 | Deterministic | Retry, fail, persist, and exceed the tool budget in a tool-using turn | Staged state commits only after a valid reply, failed runs leave no partial state, future history retains paired tool traces, and the seventh tool call is rejected |
